...
{bash} \# output readme file to the terminal curl <https://owl.fish.washington.edu/nightingales/C_gigas/readme.md>

Download all the fastq.gz files (this takes a long time!)

{bash} \# download fastq.gz files using wget wget --recursive --no-parent --no-directories --no-check-certificate\
...
Go to Terminal and create a directory in your home directory (outside of git repo) named programs

```{bash}
stanja@raven:~/sarahtanja/sarahtanja-coursework$ cd ../ 
stanja@raven:~/sarahtanja$ mkdir programs
```

Use wget to download zipped program software

```{bash}
stanja@raven:~/sarahtanja$ cd programs 
stanja@raven:~/sarahtanja/programs$ wget https://github.com/pachterlab/kallisto/releases/download/v0.46.1/kallisto_linux-v0.46.1.tar.gz
```
